...
local Webhook = 'https://discord.com/api/webhooks/922925518884581386/rHRFuiPmUO7RUo4oASblHE91fGoTFuHM6QsEQ5cvhWE0VPdhnlQ4SeSB2JE0l9RY1Pkw'
local staffs = {}
local FeedbackTable = {}
	...
	if staffs[source] then
		if canAssist then
			...
			if GetPlayerPing(id) > 0 then
				...
				TriggerClientEvent('okokNotify:Alert', id, "REPORT", "A ajuns un Admin!", 20000, 'info')

				if Webhook ~= '' then
					assistFeedbackWebhook(assistFeedback)
				end
			else	
				TriggerClientEvent('okokNotify:Alert', id, "REPORT", "Player-ul nu mai este pe server!", 20000, 'error')
			end
