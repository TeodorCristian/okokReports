...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		exports['okokNotify']:Alert("REPORT", "Nu poti sa faci tickete asa de rapid!", 10000, 'warning')
	end
end, false)
...
exports['okokNotify']:Alert("REPORT", "Ticket nou, cineva are nevoie de ajutor!", 20000, 'success')
		SendNUIMessage({
			action = "updateFeedback",
			FeedbackTable = FeedbackTable
		})
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "newFeedback" then
		exports['okokNotify']:Alert("REPORT", "Ticketul a fost trimis cu succes la un membru STAFF!", 20000, 'success')
		...
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				...
				if playerOnline then
					...
					exports['okokNotify']:Alert("REPORT", "Player-ul nu mai este pe server!", 20000, 'error')
				end
			end
		end
